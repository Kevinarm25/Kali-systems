...
const METHODS = [
  ...
  { icon: Landmark, label: "Transferencia bancaria", className: "h-3.5 w-3.5" },
  { icon: SiMercadopago, label: "Mercado Pago", className: "h-4 w-4" },
];

export default function PaymentMethods() {
  return (
    <div className="md:col-span-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-6 border-t border-white/[0.04]">
      ...
      <ul className="flex flex-wrap items-center gap-1.5 sm:gap-2">
        {METHODS.map(({ icon: Icon, label, className }) => (
          <li key={label}>
            <span
              className="group flex h-8 min-w-[2.75rem] items-center justify-center rounded-lg border border-white/[0.06] bg-white/[0.02] px-2.5 text-white/35 transition-colors duration-300 hover:border-white/12 hover:bg-white/[0.04] hover:text-white/55"
              aria-label={label}
              title={label}
            >
